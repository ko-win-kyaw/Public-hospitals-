...
auth.onAuthStateChanged(async (user) => {
    ...
    if (user) {
        try {
            // ၁။ Device ID ရယူခြင်း (Timeout ဖြင့် Error မတက်အောင် ထိန်းထားခြင်း)
            ...
            console.log("✅ Device ID:", currentDevId);

            const userRef = db.collection("users").doc(user.uid);
            const doc = await userRef.get();
            console.log("✅ User Doc Retrieved");

            // ၂။ Device Lock စစ်ဆေးခြင်း (Admin ကင်းလွတ်ခွင့်နှင့် Migration Logic ပေါင်းစပ်ထားသည်)
            if (doc.exists) {
                const existingData = doc.data();
                const isAdmin = user.email === ADMIN_EMAIL; // Admin ဟုတ်မဟုတ် စစ်ဆေးခြင်း

                /** * Admin မဟုတ်မှသာ အောက်ပါအချက်များကို စစ်ဆေးမည်-
                 * - currentDevId က timeout ဖြစ်မနေရဘူး။
                 * - Database ထဲမှာ deviceId ရှိပြီးသား ဖြစ်ရမယ်။ (User အဟောင်းများ မငြိစေရန်)
                 * - Database ထဲက ID နှင့် အခုလက်ရှိ ID မတူမှသာ Logout လုပ်မည်။
                 */
                if (!isAdmin && currentDevId !== "timeout_id" && existingData.deviceId && existingData.deviceId !== currentDevId) {
                    ...
                    await auth.signOut();
                    return; // ဒီနေရာမှာတင် ရပ်လိုက်မယ်
                }
            }

            // ၃။ Data Update လုပ်ခြင်း
            ...
            // Device ID က timeout မဖြစ်မှသာ သိမ်းဆည်းမည် (Admin ရဲ့ ID ကိုလည်း Register လုပ်ပေးသွားမှာပါ)
            ...
            // ၄။ Auto Friend System (မူရင်း Logic)
            ...
            if (!user.displayName || !isSafeName(user.displayName)) {
                ...
                if(nameDisplay) {
                    // Badge ပါသော အမည်ကို ပြသရန်
                    const userDataForBadge = doc.exists ? doc.data() : { author: currentName };
    ...
    // ၇။ ဘာပဲဖြစ်ဖြစ် Post Load လုပ်ခြင်းကို မရပ်တန့်စေရ
