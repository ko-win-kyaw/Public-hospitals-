...
function renderPostHTML(id, d, uid, isAdmin) {
    ...
    // --- Badge Logic (Post မှာ Badge ပေါ်စေရန်) ---
    let badge = "";
    if (d.isCrown) {
    ...
    // ၂။ ဗီဒီယိုနှင့် ပုံများအတွက် Logic (မူလအတိုင်း ၁၀၀%)
    let media = "";
    if (d.mediaUrl) {
    ...
    // ၃။ UI ပြသခြင်း (Flexbox Layout ဖြင့် အသစ်ပြင်ဆင်ထားသည်)
    return `
        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 10px;">
            <div style="display: flex; flex-direction: column; flex: 1; min-width: 0;">
                <b style="color: purple; font-size: 15px; display: flex; align-items: center; gap: 5px; flex-wrap: wrap;">
                    <span style="overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${d.author}</span>
                    ${badge}
                </b>
            ...
            <div style="display: flex; gap: 10px; flex-shrink: 0; margin-left: 10px;">
                ${isAdmin ? `<button onclick="togglePin('${id}', ${d.isPinned || false})" style="border:none; background:none; cursor:pointer; padding: 0;">${d.isPinned ? '📌' : '📍'}</button>` : ''}
                ${isAdmin ? `<button onclick="deletePost('${id}')" style="border:none; background:none; cursor:pointer; padding: 0;">🗑️</button>` : ''}
            </div>
        </div>
