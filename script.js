...
auth.onAuthStateChanged(async (user) => {
    ...
    if (user) {
        ...
        // --- Device Lock Logic (စင်းလုံးချော ပြင်ဆင်ချက်) ---
        const currentDevId = await getMyDeviceId();
        const userRef = db.collection("users").doc(user.uid);
        const doc = await userRef.get();

        if (doc.exists) {
            const storedDevId = doc.data().deviceId;

            // ၁။ Database မှာ Device ID ရှိပြီးသားဖြစ်ပြီး လက်ရှိဖုန်း ID နဲ့ မတူရင် တားဆီးမယ်
            if (storedDevId && storedDevId !== currentDevId) {
            ...
            // ၂။ ID မရှိသေးရင် သို့မဟုတ် တူနေရင် အချက်အလက် Update လုပ်မယ်
            // (deviceId ကို မပြောင်းလဲစေရန် merge လုပ်တဲ့အခါ deviceId ကို မထည့်တော့ပါ)
            await userRef.set({
                uid: user.uid,
                email: user.email || "",
            ...
            // ၃။ အကောင့်အသစ်ဆိုရင်တော့ လက်ရှိဖုန်း ID ကို စတင်သတ်မှတ်မယ် (First Login)
            await userRef.set({
                uid: user.uid,
                email: user.email || "",
                deviceId: currentDevId, // ဒီမှာပဲ အသေမှတ်လိုက်တာပါ
                displayName: user.displayName || "User_" + user.uid.substring(0,5),
