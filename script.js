...
async function uploadAndPost(collectionName = "health_posts") {
    // Senior ရဲ့ နဂို logic အတိုင်းပဲထားတယ်
    ...
    try {
        ...
        // targetPageId သတ်မှတ်ခြင်း (Senior ရဲ့ နဂို logic အတိုင်း)
        let finalTargetId = auth.currentUser.uid;
        if (typeof targetPageId !== 'undefined' && targetPageId) {
            finalTargetId = targetPageId;
        }

        // ၃။ Firestore သိမ်းဆည်းခြင်း (Senior ရဲ့ နဂို logic အတိုင်း)
        const finalPostData = {
...
let postsListener = null; // Listener ကို သိမ်းထားရန်

/**
 * Senior ရဲ့ နဂို loadPosts Logic ကိုထိန်းထားပြီး
 * လိုအပ်တဲ့ error handling နဲ့ UI updates များ ဖြည့်စွက်ထားပါတယ်
 */
function loadPosts(collectionName = "health_posts") {
    ...
    console.log(`🔄 Loading posts from: ${collectionName}`);

    // Loading indicator ပြသခြင်း (feed empty ဖြစ်နေမှသာ)
    if (!feed.hasChildNodes()) {
        feed.innerHTML = "<div style='text-align:center; padding:20px; color:purple;'>🔄 ပို့စ်များဖတ်နေသည်...</div>";
    }
    ...
    // အရင် listener ရှိရင် ဖြုတ်မယ်
    if (postsListener && typeof postsListener === 'function') {
        postsListener();
    }

    // Real-time listener ထားမယ်
    postsListener = query.onSnapshot(snap => {
        console.log(`📦 Snapshot received: ${snap.docChanges().length} changes`);
        
        if (snap.empty) {
            if (!feed.hasChildNodes() || feed.children.length === 0) {
                feed.innerHTML = "<p style='text-align:center; color:gray; padding:20px;'>📭 Post မရှိသေးပါ။ ပထမဆုံး Post တင်လိုက်ပါ။</p>";
            }
            return;
        }

        // Pagination Cursor သတ်မှတ်ချက် (Senior's Original Logic)
        // first time load မှသာ သတ်မှတ်မယ်
        if (!lastVisiblePost) {
            lastVisiblePost = snap.docs[snap.docs.length - 1];
            console.log("📌 Last visible post set:", lastVisiblePost.id);
        }

        // Process changes (Senior ရဲ့ နဂို processChanges ကိုသုံးမယ်)
        if (typeof processChanges === 'function') {
            processChanges(snap.docChanges(), feed);
        } else {
            console.error("processChanges function not found!");
            // Fallback: ရိုးရိုး render လုပ်မယ်
            if (snap.docChanges().length > 0) {
                feed.innerHTML = ''; // Clear first
                snap.docs.forEach(doc => {
                    if (typeof renderSinglePost === 'function') {
        ...
        feed.innerHTML = `<p style='text-align:center; color:red; padding:20px;'>❌ Error: ${err.message}</p>`;
    });

    // ၂။ Infinite Scroll Logic (Senior's Original)
    ...
    // Scroll event ကို remove လုပ်ဖို့ function ပြန်ပေးမယ်
 ...
 * Senior ရဲ့ နဂို logic ကိုထိန်းထားပြီး error handling ဖြည့်စွက်ထားပါတယ်
 */
async function loadMorePosts() {
    // Double fetch မဖြစ်အောင် စစ်ဆေးခြင်း
    if (isFetching) {
    ...
    // လက်ရှိ collection နာမည်ကို သုံးမယ် (window.currentCollection)
    const collectionToUse = window.currentCollection || "health_posts";
    
    const moreQuery = db.collection(collectionToUse)
                        .orderBy("createdAt", "desc")
    ...
    try {
        ...
        if (snap.empty) {
            ...
            // End of posts indicator (optional)
            const endMsg = document.createElement('div');
            endMsg.style = "text-align:center; color:gray; padding:10px; font-size:12px;";
            endMsg.innerText = "━━ ပို့စ်အားလုံးပြီးဆုံးပါပြီ ━━";
        ...
        lastVisiblePost = snap.docs[snap.docs.length - 1];
        console.log(`📦 Loaded ${snap.docs.length} more posts, new last: ${lastVisiblePost.id}`);
        
        // Pagination ကနေရလာတဲ့ Post တွေကို Render လုပ်မယ်
        // Senior ရဲ့ နဂို renderSinglePost ကိုသုံးမယ်
        if (typeof renderSinglePost === 'function') {
            snap.docs.forEach(doc => {
                renderSinglePost(doc.id, doc.data(), feed, "append");
            });
        } else {
            console.error("renderSinglePost function not found!");
            // Fallback: ရိုးရိုး HTML တည်ဆောက်မယ်
            snap.docs.forEach(doc => {
                const data = doc.data();
        ...
        // Observer အသစ်ပြန်ချိတ်ပေးခြင်း
        ...
        errorDiv.style = "text-align:center; color:red; padding:10px; font-size:12px;";
        errorDiv.innerText = `❌ ထပ်မံဖတ်ရှု၍မရပါ: ${e.message}`;
...
/**
 * Cleanup function - Component unmount လုပ်တဲ့အခါ သုံးရန်
 */
...
/**
 * Manual refresh - Post တွေကို ပြန်ဖတ်ရန်
 */
...
function renderPostHTML(id, d, uid, isAdmin) {
    ...
    if (images.length > 0) {
        // --- Video ဖြစ်ရင် ---
        if (d.mediaType === 'video' || (images[0] && images[0].toLowerCase().includes('.mp4'))) {
            ...
            mediaContent = `
                <div style="margin-top:10px; background:#000; border-radius:12px; overflow:hidden; aspect-ratio: 16/9; display: flex; align-items: center; border: 1px solid #eee; position: relative;">
                    <video 
                        class="post-video" 
                        id="video-${id}"
                        preload="metadata"
                        ...
                        onloadedmetadata="console.log('Video loaded: ${id}')"
                        onerror="this.style.display='none'; this.parentElement.innerHTML+='<div style=\\'color:#ff6b6b; text-align:center; padding:20px; font-size:14px;\\'>❌ ဗီဒီယိုဖွင့်၍မရပါ</div>';"
                    >
                        <source src="${finalVideoUrl}" type="video/mp4">
                        <source src="${finalVideoUrl}" type="video/webm">
                        <source src="${finalVideoUrl}" type="video/ogg">
                        သင့်ဘရောင်ဇာက ဗီဒီယိုကို ပံ့ပိုးမထားပါ။
                    </video>
                    <div style="position:absolute; top:10px; right:10px; background:rgba(0,0,0,0.6); color:white; padding:4px 8px; border-radius:15px; font-size:10px; font-weight:bold; z-index:5;">
                        🎬 Video
                    </div>
                </div>
                <div style="font-size:10px; color:gray; margin-top:5px; text-align:center; background:#f8f9fa; padding:3px; border-radius:15px;">
                    ⚡ ကြည့်ရှုရန် Play ကိုနှိပ်ပါ
                </div>`;
        } 
        // --- ပုံဖြစ်ရင် ---
        else {
            if (images.length === 1) {
                mediaContent = `
                    <div style="margin-top:10px; cursor:pointer; position:relative;" onclick="incrementView('${id}'); viewFullImage('${images[0]}')">
                        <img src="${images[0]}" 
                             style="width:100%; border-radius:12px; display:block; object-fit:cover; border: 1px solid #f0f0f0;" 
                             loading="lazy"
                             onerror="this.src='https://placehold.co/400x300?text=Image+Error'">
                        <div style="position:absolute; top:10px; right:10px; background:rgba(0,0,0,0.6); color:white; padding:4px 8px; border-radius:15px; font-size:10px;">
                            📷 Photo
                        </div>
                    </div>`;
            } else {
                ...
                <div class="media-slider-container" style="position: relative; margin-top:10px; overflow: hidden;">
                    <div class="media-slider" style="display: flex; overflow-x: auto; scroll-snap-type: x mandatory; gap: 8px; scrollbar-width: none; -ms-overflow-style: none; padding: 2px 0;">
                        ${images.map((url, index) => `
                            <div style="flex: 0 0 85%; scroll-snap-align: center; position: relative; aspect-ratio: 4/3; cursor: pointer; border-radius:12px; overflow:hidden;" 
                                ...
                                <img src="${url}" 
                                     ...
                                     loading="lazy"
                                     onerror="this.src='https://placehold.co/400x300?text=Image+Error'">
                                <div style="position:absolute; top:10px; right:10px; background:rgba(0,0,0,0.6); color:white; padding:4px 8px; border-radius:15px; font-size:10px; font-family: sans-serif;">
                    ...
                    <div style="display:flex; justify-content:center; gap:5px; margin-top:8px;">
                        ${images.map((_, index) => `
                            <span style="width:6px; height:6px; border-radius:50%; background:#ccc; display:inline-block;"></span>
                        `).join('')}
                    </div>
                </div>`;
            }
    ...
    // Text ကို safe ဖြစ်အောင်
    const safeText = (d.text || "").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/\n/g, '<br>');

    // Return ပြန်မယ့် HTML
    return `
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
            ...
            <div style="display: flex; gap: 15px;">
                ${isAdmin ? `<span onclick="togglePin('${id}', ${d.isPinned || false})" style="cursor:pointer; font-size:18px; transition:0.2s;" title="${d.isPinned ? 'Unpin' : 'Pin'}">${d.isPinned ? '📌' : '📍'}</span>` : ''}
                ${isAdmin ? `<span onclick="deletePost('${id}')" style="cursor:pointer; font-size:18px; transition:0.2s;" title="Delete">🗑️</span>` : ''}
            </div>
        </div>
        ...
        <div style="display: flex; justify-content: space-between; margin-top: 15px; border-top: 1px solid #f0f2f5; padding-top: 12px;">
            ...
            <div class="stat-content" style="font-size: 12px; color: #65676b; display: flex; align-items: center; gap: 8px;">
                <span style="display:flex; align-items:center; gap:2px;">👁️ ${d.views||0}</span>
                <span style="color: #ddd;">|</span>
                <span onclick="handleShare('${id}')" style="cursor:pointer; color:purple; font-weight:bold; background: #f3e5f5; padding: 4px 10px; border-radius: 20px; display:flex; align-items:center; gap:3px; transition:0.2s;">
                    🚀 <span>${d.shares||0}</span>
                </span>
            </div>
        </div>

        <div style="margin-top: 12px; background: #f0f2f5; border-radius: 15px; padding: 10px; border: 1px solid #e4e6eb;">
            <div id="comms-${id}" style="max-height: 250px; overflow-y: auto; scroll-behavior: smooth; padding-right: 5px;">
            ...
            <div style="display: flex; gap: 8px; padding: 8px 4px 4px 4px; border-top: 1px solid #e4e6eb; margin-top: 8px;">
                <input type="text" id="in-${id}" placeholder="မှတ်ချက်ပေးရန်..." 
                       ...
                       style="flex:1; border-radius: 25px; border: 1px solid #ddd; padding: 10px 15px; font-size: 13.5px; outline: none; background: white; transition:0.2s;">
                <button onclick="addComment('${id}')" style="color: white; border: none; background: purple; font-weight: bold; cursor: pointer; padding: 0 20px; border-radius: 25px; font-size: 13px; transition:0.2s;">ပို့</button>
            </div>
        </div>`;
}
    ...
    const getCommentRow = (c, i, isHidden = false) => {
        ...
        <div style="background:#f0f2f5; margin-bottom:6px; padding:10px; border-radius:12px; font-size:12.5px; text-align:left; border-left:${isTop?'4px solid #ffd700':''}; transition: 0.3s;">
            ...
            <div style="margin-top:6px; display:flex; gap:15px; align-items:center;">
                <span onclick="reactComment('${postId}', ${c.createdAt}, 'likes')" style="cursor:pointer; font-weight:bold; color:${(c.likedBy||[]).includes(uid)?'#1877f2':'#65676b'}">👍 ${c.likes||0}</span>
<span onclick="reactComment('${postId}', ${c.createdAt}, 'hahas')" style="cursor:pointer; font-weight:bold; color:${(c.hahaedBy||[]).includes(uid)?'#f7b125':'#65676b'}">😆 ${c.hahas||0}</span>
...
// Advanced Preview with Clear Button
function previewMediaAdvanced(input) {
    ...
    if (file) {
        ...
        // Clear Button ပါတဲ့ Preview Box
        let content = '';
        
        if (file.type.startsWith('video/')) {
            content = `
                <video src="${currentPreviewUrl}" style="width:100%; border-radius:8px;" controls muted loop></video>
                <div style="display:flex; justify-content:space-between; align-items:center; margin-top:5px;">
                    <small style="color:gray;">🎬 Video • ${(file.size/(1024*1024)).toFixed(2)} MB</small>
                    <button onclick="clearPreview()" style="background:#ff4444; color:white; border:none; padding:2px 10px; border-radius:15px; cursor:pointer; font-size:11px;">✕ ဖျက်မည်</button>
                </div>`;
        } else {
            content = `
                <img src="${currentPreviewUrl}" style="width:100%; border-radius:8px; display:block;">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-top:5px;">
                    <small style="color:gray;">📷 Image • ${(file.size/1024).toFixed(0)} KB</small>
                    <button onclick="clearPreview()" style="background:#ff4444; color:white; border:none; padding:2px 10px; border-radius:15px; cursor:pointer; font-size:11px;">✕ ဖျက်မည်</button>
                </div>`;
        }
...
// Mobile Browser detect လုပ်ဖို့
function isMobile() {
    return /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
}

// Mobile ဆိုရင် video settings ပြောင်းမယ်
if (isMobile()) {
    // Mobile မှာ autoplay ကို disable လုပ်မယ်
    videoObserver.threshold = [0.8]; // ပိုများတဲ့ threshold ထားမယ်
    
    // Video တွေအားလုံးကို controls ပါအောင်လုပ်မယ်
...
// Video တစ်ခုချင်းစီကို Play/Pause လုပ်ဖို့
...
// Page ပြောင်းတဲ့အခါ video တွေအားလုံး pause လုပ်ဖို့
...
// Video ကို fullscreen ဖွင့်ဖို့
...
// Senior ရဲ့ နဂို export တွေကိုထိန်းထားပြီး uploadAndPost ကိုထည့်မယ်
...
window.clearPreview = clearPreview; // ဒါကိုထည့်ဖို့လိုတယ်
window.viewFullImage = viewFullImage; 
window.incrementView = incrementView;
window.submitFeedback = submitFeedback;
window.ADMIN_EMAIL = ADMIN_EMAIL;
// Senior ရဲ့ နဂို functions တွေကိုထိန်းထားပြီး အသစ်ထည့်မယ်
window.loadPosts = loadPosts;
window.loadMorePosts = loadMorePosts;
window.cleanupPosts = cleanupPosts;
window.refreshPosts = refreshPosts;
window.lastVisiblePost = lastVisiblePost; // လိုအပ်ရင်
window.isFetching = isFetching; // လိုအပ်ရင်
})();
