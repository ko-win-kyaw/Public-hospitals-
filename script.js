...
// Image Viewer အတွက် HTML ကို Body ထဲ ထည့်သွင်းခြင်း
...
function renderPostHTML(id, d, uid, isAdmin) {
    ...
    // ===== MEDIA RENDERING (Full Width - မူရင်း Logic အတိုင်း) =====
    let media = "";
    
    // Case 1: mediaUrls array ရှိပြီး ဖိုင်တွေပါရင်
    if (d.mediaUrls && d.mediaUrls.length > 0) {
        if (d.mediaType === 'video') {
            // Video တစ်ခုတည်းဆိုရင်
            let finalVideoUrl = d.mediaUrls[0];
            // Bunny.net URL ပြင်ခြင်း (မူရင်း Logic အတိုင်း)
            ...
            media = `
                <div style="margin-top:10px; background:#000; width:100%; margin-left:0; margin-right:0;">
                    <video 
                        src="${finalVideoUrl}" 
            ...
            // ပုံအများကြီးဆိုရင် Grid နဲ့ပြမယ်
            media = `<div style="display: grid; grid-template-columns: ${d.mediaUrls.length > 1 ? 'repeat(2, 1fr)' : '1fr'}; gap: 2px; margin-top: 10px; width:100%;">`;
            d.mediaUrls.forEach(url => {
                media += `<img 
                    src="${url}" 
                    style="width:100%; height:250px; object-fit:cover; cursor:pointer;" 
                    onclick="incrementView('${id}')"
                >`;
            });
            media += `</div>`;
        }
    } 
    // Case 2: mediaUrl တစ်ခုတည်းရှိရင် (မူရင်း Post များအတွက်)
    else if (d.mediaUrl) {
        if (d.mediaType === 'video' || d.mediaUrl.toLowerCase().includes('.mp4')) {
            ...
            media = `<img 
                onclick="incrementView('${id}')" 
                src="${d.mediaUrl}" 
                style="width:100%; margin-top:10px; display:block; cursor:pointer;"
            >`;
        }
    }

    // ===== ORIGINAL UI STRUCTURE (လုံးဝမပြင်ဘူး) =====
    return `
...
auth.onAuthStateChanged(async (user) => {
    ...
    // User မရှိရင် (Logout)
    if (!user) {
        // လိုအပ်ရင် UI ကို Guest mode ပြောင်း
        return;
    }

    // User ရှိနေရင် အောက်ပါအတိုင်း ဆက်လုပ်မယ်
    try {
        // ၁။ Device ID ရယူခြင်း (အားလုံးအတွက် တစ်ခါတည်းယူ)
        const currentDevId = await Promise.race([
            getMyDeviceId(),
            new Promise(resolve => setTimeout(() => resolve("timeout_id"), 5000))
        ]);

        // ၂။ Ban Status စစ်ဆေးခြင်း (အရေးကြီးဆုံး)
        const isBanned = await checkBanStatus(user.uid, currentDevId);
        if (isBanned) {
            await auth.signOut();
            return; // ဒီမှာရပ်လိုက်မယ်
        }

        // ၃။ Device Lock စစ်ဆေးခြင်း (Admin ကလွဲရင်)
        ...
        // ၅။ နာမည်မရှိသေးရင် Name Setup Modal ပြခြင်း
        if (!user.displayName) {
            const modal = document.getElementById('nameSetupModal');
            if (modal) modal.style.display = 'flex';
        }

        // ၆။ Auto Friend System ကို နောက်ခံမှာ စတင်ခြင်း (သေချာအောင် try/catch)
        if (user.uid) {
            // Firestore ထဲက isAutoFriendAdded ကို function ထဲမှာ စစ်ပြီးသားမို့ ဒီမှာ ထပ်မစစ်တော့
            startAutoFriendSystem(user.uid).catch(err => 
                console.error("Auto friend system error:", err)
            );
        }

        // ၇။ Live Notifications စတင်ခြင်း
        startLiveNotifications();

    } catch (error) {
        console.error("Auth State Handler Error:", error);
        // လိုအပ်ရင် user ကို အသိပေးနိုင်တယ်
        // alert("System error occurred. Please refresh.");
    }
});
