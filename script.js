...
auth.onAuthStateChanged(async (user) => {
    ...
    if (user) {
        try {
            // ၁။ Device ID ရယူခြင်း (Timeout ထည့်ထားပါတယ်)
            console.log("🔍 Fetching Device ID...");
            const currentDevId = await Promise.race([
                getMyDeviceId(),
                new Promise(resolve => setTimeout(() => resolve("timeout_id"), 5000)) // 5 စက္ကန့်ထက်ကျော်ရင် ကျော်သွားမယ်
            ]);
            ...
            if (doc.exists) {
                ...
                if (existingData.deviceId && existingData.deviceId !== currentDevId && currentDevId !== "timeout_id") {
            ...
            if (!user.displayName || !isSafeName(user.displayName)) {
                ...
                if(nameDisplay) {
                    // Badge ကို တစ်ခါတည်းတွဲပြတဲ့ Logic
                    nameDisplay.innerHTML = getDisplayNameWithBadge(doc.exists ? doc.data() : {author: user.displayName});
                }
            }

        } catch (e) {
            console.error("❌ Auth Error:", e);
            // Error တက်လည်း Loading မရပ်အောင် loadPosts ကို ခေါ်ပေးမယ်
        }
    } else {
        if (nameDisplay) nameDisplay.innerText = "ဧည့်သည် (Guest)";
    }
    
    // ၅။ ဘာပဲဖြစ်ဖြစ် Post ကိုတော့ တင်ပေးမယ်
