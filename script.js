...
async function submitFeedback() {
    ...
    try {
        ...
        // User post အဖြစ်လည်း တင်ပေးခြင်း
        await db.collection("health_posts").add({ 
            author: userName, 
            text: `⭐ App Rating: ${selectedRatingValue}/5\n"${fbText}"`, 
...
// ၅။ Main App (Pin, Delete, Comment စနစ်များ)
function startApp() {
    db.collection("health_posts").orderBy("isPinned", "desc").orderBy("createdAt", "desc").onSnapshot(snap => {
        ...
        snap.forEach(doc => {
            ...
            let commentsHtml = (d.comments || []).map(c => `
                <div style="border-bottom:1px solid #eee; padding:8px 0; font-size:14px;">
                    <div style="display:flex; justify-content:space-between;">
                        <b>${c.author}</b>
                        ${isAdmin ? `<span style="color:red; cursor:pointer; font-size:10px;" onclick="deleteComment('${id}','${c.id}')">🗑️</span>` : ''}
                    </div>
            ...
            feed.innerHTML += `
                <div class="card ${d.isPinned ? 'pinned' : ''}" style="border:1px solid #ddd; padding:15px; border-radius:12px; margin-bottom:20px; background:#fff;">
                    ${adminUI}
                    ...
                    ${d.videoUrl ? `<video controls playsinline style="width:100%; border-radius:8px; margin-bottom:10px; background:#000;"><source src="${d.videoUrl}" type="video/mp4"></video>` : ''}
                    <div class="action-bar" style="display:flex; gap:10px; border-top:1px solid #eee; padding-top:10px;">
                        <button onclick="reactPost('${id}', 'likes')" style="flex:1; border:none; padding:8px; border-radius:5px; ${(d.likedBy||[]).includes(uid)?'color:blue;font-weight:bold;':''}">👍 Like (${d.likes||0})</button>
                        <button onclick="reactPost('${id}', 'hahas')" style="flex:1; border:none; padding:8px; border-radius:5px; ${(d.hahaedBy||[]).includes(uid)?'color:orange;font-weight:bold;':''}">😆 Haha (${d.hahas||0})</button>
                    </div>
                    <div style="margin-top:15px; background:#fafafa; padding:10px; border-radius:8px;">
                        ${commentsHtml}
                        <div style="display:flex; gap:5px; margin-top:10px;">
                            <input type="text" id="in-${id}" placeholder="မှတ်ချက်..." style="flex:1; border-radius:20px; border:1px solid #ddd; padding:5px 12px;">
...
// ၆။ Media Upload (Image & Video up to 20MB+)
    ...
    try {
        ...
        if(vidFile) {
            ...
            const res = await fetch(`https://${BUNNY_REGION}.storage.bunnycdn.com/${BUNNY_ZONE}/${fileName}`, {
                method: "PUT", headers: { "AccessKey": BUNNY_STORAGE_KEY, "Content-Type": "application/octet-stream" }, body: vidFile
            });
            if(res.status === 201 || res.status === 200) videoUrl = `https://${BUNNY_PULL_ZONE}/${fileName}`;
        }
        await db.collection("health_posts").add({ 
        ...
        location.reload();
    } catch (e) { alert("Upload Error: " + e); btn.disabled = false; }
}

// ၇။ Actions (Delete, Pin, Comment Reaction)
