...
function previewMedia(input) {
    ...
    previewBox.innerHTML = ""; // အရင်ရှိနေတဲ့ Preview ကို ရှင်းထုတ်တာ
    ...
    if (input.files && input.files[0]) {
        ...
        reader.onload = function(e) {
            ...
            if (file.type.startsWith('image/')) {
                // ပုံဆိုရင် img tag နဲ့ ပြမယ်
                previewBox.innerHTML = `
                    <div style="position:relative; display:inline-block;">
                        <img src="${e.target.result}" style="max-width:100%; max-height:200px; border-radius:8px; border:2px solid purple;">
                        <span onclick="clearPreview()" style="position:absolute; top:-10px; right:-10px; background:red; color:white; border-radius:50%; width:20px; height:20px; cursor:pointer; line-height:20px; text-align:center;">×</span>
                    </div>`;
            } else if (file.type.startsWith('video/')) {
                // ဗီဒီယိုဆိုရင် video tag နဲ့ ပြမယ်
                previewBox.innerHTML = `
                    <div style="position:relative; display:inline-block; width:100%;">
                        <video src="${e.target.result}" style="width:100%; max-height:200px; border-radius:8px; border:2px solid purple;" muted></video>
                        <span onclick="clearPreview()" style="position:absolute; top:-10px; right:5px; background:red; color:white; border-radius:50%; width:20px; height:20px; cursor:pointer; line-height:20px; text-align:center;">×</span>
                        <p style="font-size:10px; color:gray;">ဗီဒီယို Preview (အသံပိတ်ထားသည်)</p>
                    </div>`;
            }
        };
...
// Preview ကို ပြန်ဖျက်တဲ့ function
