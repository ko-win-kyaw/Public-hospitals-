...
async function loadPosts(collectionName = "health_posts") {
    try {
        ...
        // ၁။ Login အခြေအနေကို ကြိုစစ်ထားခြင်း (Guest အတွက်ပါ အဆင်ပြေစေရန်)
        const currentUid = auth.currentUser ? auth.currentUser.uid : null;
        const isAdmin = auth.currentUser ? (auth.currentUser.email === ADMIN_EMAIL) : false;

        // ၂။ Pin post နှင့် အချိန်အလိုက် စီခြင်း (Composite Index လိုအပ်သည်)
        ...
        snapshot.forEach(doc => {
            const post = doc.data();
            // ၃။ renderPostHTML သို့ Data အစုံပို့ပေးခြင်း
            html += renderPostHTML(doc.id, post, currentUid, isAdmin);
        });
        
        // ၄။ Load More Button (Infinite Scroll Trigger အဖြစ်ပါ အလုပ်လုပ်နိုင်သည်)
        html += `<div id="scroll-trigger" style="text-align:center; margin:20px;">
            <button onclick="loadMorePosts('${collectionName}')" style="background:purple; color:white; border:none; padding:10px 20px; border-radius:5px; cursor:pointer;"> 
                Load More Posts 
            </button>
        </div>`;
        ...
        // ၅။ ပို့စ်တွေပေါ်လာပြီးနောက် Video နှင့် View Observer များကို အလုပ်ပေးခြင်း
        setTimeout(() => {
            // Video Auto-play observer
            if (window.videoObserver) {
                document.querySelectorAll('video').forEach(v => videoObserver.observe(v));
            }
            // ၆၀% မြင်ရင် View တိုးမည့် Observer
        ...
        // Error Message ကို Senior ဖတ်ရလွယ်အောင် ပြင်ထားပါတယ်
...
function renderPostHTML(id, d, uid, isAdmin) {
    ...
    // ဗီဒီယို မဲမနေစေဘဲ ပုံရိပ် (Thumbnail) ပေါ်စေရန် URL ပြုပြင်ခြင်း
    const getSafeVideoUrl = (url) => {
        ...
        // #t=0.001 ထည့်ပေးခြင်းဖြင့် ဗီဒီယိုရဲ့ ပထမဆုံး Frame ကို ပုံအဖြစ် ပြပေးမှာပါ
        return finalUrl.includes('#t=') ? finalUrl : `${finalUrl}#t=0.001`;
    };

    // --- PHOTO GRID & VIDEO LOGIC ---
    if (d.mediaUrls && d.mediaUrls.length > 0) {
        if (d.mediaType === 'video') {
            media = `
                <div style="margin-top:10px; background:#000; border-radius:8px; overflow:hidden;">
                    <video src="${getSafeVideoUrl(d.mediaUrls[0])}" controls preload="metadata" playsinline webkit-playsinline style="width:100%; display:block; min-height:200px; background:#000;"></video>
                </div>`;
        } else {
            ...
            for (let i = 0; i < displayCount; i++) {
                ...
                media += `
                    <div class="grid-item" onclick="openPhotoViewer(${i}, '${photosJson}')">
        ...
        if (d.mediaType === 'video' || d.mediaUrl.toLowerCase().includes('.mp4')) {
            media = `
                <div style="margin-top:10px; background:#000; border-radius:8px; overflow:hidden;">
                    <video src="${getSafeVideoUrl(d.mediaUrl)}" controls preload="metadata" playsinline webkit-playsinline style="width:100%; display:block; min-height:200px; background:#000;"></video>
                </div>`;
        } else {
            const singlePhotoJson = encodeURIComponent(JSON.stringify([d.mediaUrl]));
            media = `<img onclick="openPhotoViewer(0, '${singlePhotoJson}')" src="${d.mediaUrl}" style="${originalViewStyle}">`;
        }
    }
...
function getDisplayNameWithBadge(d) {
    let badges = ""; // Badge များကို စုသိမ်းရန် variable

    // ၁။ Crown badge ရှိမရှိ စစ်ဆေးပြီး ရှိလျှင် ပေါင်းထည့်မည်
    if (d.isCrown) {
        badges += `<span class="badge-official crown-bg" style="margin-left: 5px;">👑 Crown</span>`;
    }

    // ၂။ Gold badge ရှိမရှိ ထပ်မံစစ်ဆေးပြီး ရှိလျှင် ထပ်မံပေါင်းထည့်မည်
    if (d.isGold) {
        badges += `<span class="badge-official gold-bg" style="margin-left: 5px;">⭐ Gold</span>`;
    }

    // နဂိုအတိုင်း Author Name နဲ့ အတူ Badge များကို ပြန်ပေးမည်
    return `${d.author || "User"} ${badges}`;
}
...
function openPhotoViewer(index, photosJson) {
    try {
        currentPhotos = JSON.parse(decodeURIComponent(photosJson));
        currentIndex = index;
        
        const viewer = document.getElementById('photoViewer');
        const img = document.getElementById('activeImg'); // ဒီ ID ကို သေချာအောင် စစ်ပါ
        
        if (viewer && img) {
            img.src = currentPhotos[currentIndex];
            viewer.style.display = 'flex'; 
            document.body.style.overflow = 'hidden'; // Scroll ပိတ်ရန်
        }
    } catch (e) {
...
function changeSlide(n) {
    ...
    document.getElementById('activeImg').src = currentPhotos[currentIndex];
}
...
function closePhotoViewer() {
    ...
    if (viewer) {
        ...
        document.body.style.overflow = 'auto'; // scroll ပြန်ဖွင့်ခြင်း
    }
}
