    ...
    db.collection("health_posts").orderBy("createdAt", "desc").onSnapshot(snap => {
        ...
        snap.docChanges().forEach(change => {
            ...
            if (change.type === "added" && !postEl) {
                ...
                div.id = `post-${id}`; div.className = "post-card";
            ...
            else if (change.type === "removed" && postEl) { postEl.remove(); }
        });
        observeElements();
    });
}
...
function renderPostHTML(id, d, uid, isAdmin) {
    ...
    return `
        ...
        <div style="display:flex; justify-content:space-between; margin-top:10px; border-top:1px solid #eee; padding-top:10px;">
            ...
            <div style="font-size:12px; color:gray;">👁️ ${d.views||0} | <span onclick="handleShare('${id}')" style="cursor:pointer; color:purple;">🚀 Share (${d.shares||0})</span></div>
        </div>
