...
async function loadPosts(collectionName = "health_posts") {
    try {
        ...
        postsContainer.innerHTML = '<div style="text-align:center; padding:20px;">⏳ ပို့စ်များ ဖတ်နေသည်...ခဏစောင့်ပါ...</div>';
    ...
    if (d.mediaUrls && d.mediaUrls.length > 0) {
        if (d.mediaType === 'video') {
            ...
            d.mediaUrls.forEach(url => {
                media += `<img src="${url}" style="${originalViewStyle}" onclick="viewFullImage('${url}')">`;
            });
            media += `</div>`;
        }
    } 
        ...
        if (d.mediaType === 'video' || d.mediaUrl.toLowerCase().includes('.mp4')) {
            ...
            media = `<img onclick="viewFullImage('${d.mediaUrl}')" src="${d.mediaUrl}" style="${originalViewStyle}">`;
        }
    }

    // ပြင်ဆင်ချက်- data-id="${id}" ကို ပထမဆုံး div မှာ ထည့်သွင်းထားပါတယ်
    return `
